...
class nCallBack(nandbox.Callback):
    ...
    def on_payment_authorization_request(self,payment_request):
        napi.submit_payment_result(app_id=appId,chat_id=appId,user_id=payment_request.account_id,order_id=payment_request.order_id,total_amount=payment_request.amount,currency=payment_request.currency,provider_response=payment_request.payload,secret=payment_request.secret,status="Success",debit_amount_cents=payment_request.debit_amount_cents)
